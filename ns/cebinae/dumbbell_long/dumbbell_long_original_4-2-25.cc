...
main (int argc, char *argv[])
{
  ...
  // Create a random variable
    ...
    // // Generate random value between 0 and 0.2
    // double randomNumber = random->GetValue();

    // std::cout << randomNumber << std::endl;

// create two random variables for flow 2 and 3
Ptr<UniformRandomVariable> flowTwoRandom = CreateObject<UniformRandomVariable>(); // change to uniform random variable
flowTwoRandom->SetAttribute("Min", DoubleValue(-0.2)); // Set the mean value for onTime
flowTwoRandom->SetAttribute("Max", DoubleValue(0.2)); // Set the bound value for onTime

Ptr<UniformRandomVariable> flowThreeRandom = CreateObject<UniformRandomVariable>();
flowThreeRandom->SetAttribute("Min", DoubleValue(-0.2)); // Set the mean value for onTime
flowThreeRandom->SetAttribute("Max", DoubleValue(0.2)); // Set the bound value for onTime

double flowTwoRandomStart = app_seconds_start1 + flowTwoRandom->GetValue(); // add random value to start time
double flowThreeRandomStart = app_seconds_start2 + flowThreeRandom->GetValue();
std::cout << "app_seconds_start1: " << app_seconds_start1 << " flowTwoRandomStart: " << flowTwoRandomStart << std::endl;
std::cout << "app_seconds_start2: " << app_seconds_start2 << " flowThreeRandomStart: " << flowThreeRandomStart << std::endl;
  ...
  Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue (62914560)); // 62914560
  Config::SetDefault ("ns3::TcpSocket::SndBufSize", UintegerValue (41943040)); // 41943040
 ...
 // Use ExponentialRandomVariable for onTime and offTime
  ...
  //     app->SetStartTime (Seconds (flowTwoRandomStart));
  ...
  //     app->SetStartTime (Seconds (flowThreeRandomStart));
  ...
  // }
    ...
    // original tcp-bbr-example sink and source
  // Select senders side port
  uint16_t port = 8080;

  // Install application on the senders
  BulkSendHelper source1 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(0), port));
  source1.SetAttribute ("MaxBytes", UintegerValue (0));
  ApplicationContainer sourceApps1 = source1.Install (leftleaf.Get (0));
  sourceApps1.Start (Seconds (app_seconds_start0));
  // Simulator::Schedule (Seconds (0.2), &TraceCwnd, 0, 0);
  sourceApps1.Stop (Seconds (app_seconds_end0));

  BulkSendHelper source2 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(1), port));
  source2.SetAttribute ("MaxBytes", UintegerValue (0));
  ApplicationContainer sourceApps2 = source2.Install (leftleaf.Get (1));
  sourceApps2.Start (Seconds (flowTwoRandomStart));
  // Simulator::Schedule (Seconds (0.2), &TraceCwnd, 0, 0);
  sourceApps2.Stop (Seconds (app_seconds_end1));

  BulkSendHelper source3 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(2), port));
  source3.SetAttribute ("MaxBytes", UintegerValue (0));
  ApplicationContainer sourceApps3 = source3.Install (leftleaf.Get (2));
  sourceApps3.Start (Seconds (flowThreeRandomStart));
  // Simulator::Schedule (Seconds (0.2), &TraceCwnd, 0, 0);
  sourceApps3.Stop (Seconds (app_seconds_end2));

  BulkSendHelper source4 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(3), port));
  source4.SetAttribute ("MaxBytes", UintegerValue (0));
  ApplicationContainer sourceApps4 = source4.Install (leftleaf.Get (3));
  sourceApps4.Start (Seconds (app_seconds_start3));
  // Simulator::Schedule (Seconds (0.2), &TraceCwnd, 0, 0);
  sourceApps4.Stop (Seconds (app_seconds_end3));

  // Install application on the receivers
  PacketSinkHelper sink1 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(0), port));
  ApplicationContainer sinkApps1 = sink1.Install (rightleaf.Get (0));
  sinkApps1.Start (Seconds (app_seconds_start));
  sinkApps1.Stop (Seconds (app_seconds_end));

  PacketSinkHelper sink2 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(1), port));
  ApplicationContainer sinkApps2 = sink2.Install (rightleaf.Get (1));
  sinkApps2.Start (Seconds (app_seconds_start));
  sinkApps2.Stop (Seconds (app_seconds_end));

  PacketSinkHelper sink3 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(2), port));
  ApplicationContainer sinkApps3 = sink3.Install (rightleaf.Get (2));
  sinkApps3.Start (Seconds (app_seconds_start));
  sinkApps3.Stop (Seconds (app_seconds_end));

  PacketSinkHelper sink4 ("ns3::TcpSocketFactory", InetSocketAddress (rightleaf_ifc.GetAddress(3), port));
  ApplicationContainer sinkApps4 = sink4.Install (rightleaf.Get (3));
  sinkApps4.Start (Seconds (app_seconds_start));
  sinkApps4.Stop (Seconds (app_seconds_end));
  ...
  for (uint16_t sourceid = 0; sourceid < num_leaf; sourceid++) {
    ...
    if (logtcp) {
      if (queuedisc_type.compare("FifoQueueDisc") == 0) {
        ...
        // }
        // std::ofstream cwnd_ofs (result_dir + "/FIFO-cwnd_"+std::to_string(sourceid)+".dat", std::ios::out | std::ios::app);
      ...
      if (queuedisc_type.compare("CebinaeQueueDisc") == 0) {
        ...
        //   rtt_ofs << sourceidtag2rttlog[sourceid][i] << "\n";
        // }
        // std::ofstream cwnd_ofs (result_dir + "/Cebinae-cwnd_"+std::to_string(sourceid)+".dat", std::ios::out | std::ios::app);
        // int num_cwnd_samples = sourceidtag2cwndlog[sourceid].size();
        // for (int i = 0; i < num_cwnd_samples; i++) {
        //   cwnd_ofs << sourceidtag2cwndlog[sourceid][i] << "\n";
        // }
